<template>
  <div id="app">
    ...
    <!-- ── Sign-in screen ── -->
    <div v-else-if="appState === 'signin'" class="splash">
      <div class="splash-logo">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
          <circle cx="12" cy="12" r="9"/>
          <path d="M10 8.5l5 3.5-5 3.5V8.5z" fill="currentColor" stroke="none"/>
      ...
      <p class="splash-name">Moin<em>Tube</em></p>
      <p class="splash-subtitle">Your private YouTube feed</p>

      <button class="btn-google" @click="doSignIn" :disabled="signingIn">
        <svg v-if="!signingIn" viewBox="0 0 24 24" width="18" height="18">
      ...
      <p class="splash-note">Your data is stored privately under your Google account. Clearing browser history won't affect it.</p>
    </div>
      ...
      <main class="app-main">
        ...
        <div v-show="activeTab === 'settings'" class="settings-inner">
          ...
          <div class="account-bar">
            <img v-if="userPhoto" :src="userPhoto" class="account-avatar" referrerpolicy="no-referrer" />
            <div v-else class="account-avatar account-avatar--placeholder">
            ...
            <div class="account-info">
              <span class="account-name">{{ userName || 'Signed in' }}</span>
              <span class="account-email">{{ userEmail }}</span>
            </div>
            <button class="btn-signout" @click="doSignOut" title="Sign out">
...
import {
  getCurrentUser, signInWithGoogle, handleRedirectResult, fbSignOut,
...
// ─── App state ───────────────────────────────────────────────────
const appState   = ref('init')   // 'init' | 'signin' | 'ready' | 'error'
const initError  = ref('')
const uid        = ref(null)
...
async function doSignIn() {
  ...
  try {
    ...
    // if null → redirect flow initiated, page will reload
  } catch (e) {
    signInError.value = e.message || 'Sign-in failed. Please try again.'
    signingIn.value   = false
  }
}

async function doSignOut() {
  if (!confirm('Sign out? You can sign back in any time to restore your data.')) return
  await fbSignOut()
  uid.value      = null
  userName.value = ''
  userEmail.value= ''
...
async function bootApp(user) {
  ...
  const [cfg, chs, prog, hist, anl, savedData] = await Promise.all([
    fbGetConfig(user.uid), fbGetChannels(user.uid),
    fbGetProgress(user.uid), fbGetHistory(user.uid),
    fbGetAnalytics(user.uid), fbGetSaved(user.uid)
  ])
...
onMounted(async () => {
  ...
  try {
    ...
    if (redirectUser) { await bootApp(redirectUser); return }
...
/* Sign-in screen */
.splash-subtitle{font-size:.85rem;color:var(--text-muted);margin-top:-8px}
.splash-note{font-size:.72rem;color:var(--text-faint);max-width:280px;text-align:center;line-height:1.6;margin-top:4px}
.signin-error{font-size:.76rem;color:var(--danger);background:rgba(224,80,80,.08);border:1px solid rgba(224,80,80,.2);border-radius:var(--radius);padding:8px 14px;max-width:280px;text-align:center}

.btn-google{display:flex;align-items:center;gap:10px;padding:0 24px;height:46px;border-radius:var(--radius);border:1px solid var(--border);background:var(--surface);color:var(--text-primary);font-family:var(--font-body);font-size:.88rem;font-weight:500;cursor:pointer;transition:border-color .15s,background .15s;margin-top:8px}
.btn-google:hover:not(:disabled){border-color:var(--accent);background:var(--card-bg)}
.btn-google:disabled{opacity:.6;cursor:not-allowed}
.btn-spinner{width:18px;height:18px;border-radius:50%;border:2px solid var(--border);border-top-color:var(--accent);animation:spin .7s linear infinite;flex-shrink:0}

/* Account bar in settings */
.account-bar{display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--card-bg);border:1px solid var(--border);border-radius:var(--radius)}
.account-avatar{width:36px;height:36px;border-radius:50%;flex-shrink:0;border:1px solid var(--border)}
.account-avatar--placeholder{background:var(--surface);display:flex;align-items:center;justify-content:center;color:var(--text-faint)}
.account-avatar--placeholder svg{width:18px;height:18px}
.account-info{flex:1;min-width:0;display:flex;flex-direction:column;gap:2px}
.account-name{font-size:.86rem;font-weight:500;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.account-email{font-size:.7rem;color:var(--text-faint);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.btn-signout{display:flex;align-items:center;gap:6px;padding:0 12px;height:32px;border-radius:var(--radius);border:1px solid var(--border);background:transparent;color:var(--text-muted);font-family:var(--font-body);font-size:.74rem;cursor:pointer;flex-shrink:0;transition:color .15s,border-color .15s,background .15s}
.btn-signout svg{width:14px;height:14px}
.btn-signout:hover{color:var(--danger);border-color:rgba(224,80,80,.4);background:rgba(224,80,80,.06)}
